...
- [Compiler 2020 Manual](#compiler-2020-manual)
  ...
  * [**文法规则：**](#文法规则)
    ...
    + [**8** **类：**](#8-类)
      ...
      - [**8.3 类成员访问**](#83-类成员访问)
      - [**8.4 类构造函数**](#84-类构造函数)
      - [**8.5 this指针**](#85-this指针)
      - [**8.6 类之中的未定义行为**](#86-类之中的未定义行为)
    + [**9 函数：**](#9-函数)
      - [**9.1 函数定义**](#91-函数定义)
      - [**9.2 内建函数**](#92-内建函数)
      - [**9.3 函数返回值**](#93-函数返回值)
    + [**10 表达式：**](#10-表达式)
      - [**10.1 单目表达式**](#101-单目表达式)
      - [**10.2 双目表达式**](#102-双目表达式)
    + [**11 语句：**](#11-语句)
    ...
    + [**12 字符串：**](#12-字符串)
      ...
      - [**12.2 字符串的双目运算**](#122-字符串的双目运算)
      - [**12.3 字符串的内建方法**](#123-字符串的内建方法)
    + [**13** **作用域：**](#13-作用域)
    + [**14** **命名空间：**](#14-命名空间)
    + [**15** **左值：**](#15-左值)
  * [编译规则：](#编译规则)
...
Created: 2019-10-13	Last Update: 2020-02-26v6.1	*Pre-release - v6.1*
...
3. 你的代码不应该调用除了 Parser 以外的第三方库用于任何阶段（解释：ANTLR等），例如生成LLVM IR后直接调用clang/llvm工具生成是非法的，如果自己实现了生成工具除外。
...
**（版本6.1 新增：无显式构造函数的类有一个和类名相同的默认构造函数，默认构造函数什么都不做。）**
...
如果函数声明的返回值不是`void`，就必须有`return`语句返回函数返回值，反之未定义。（例外：`main`函数例外，可以没有返回值，此时返回值为0。）
...
所有符号共享一个命名空间，所以在同一个作用域里，变量，函数，和class，都不能同名如果重名视为语法错误，注意作用域规则。简单来说，变量和函数是可以重名的，但是类是不可以和变量重名的。
...
Section 8.4：新增：无显式构造函数的类有一个和类名相同的默认构造函数，默认构造函数什么都不做。

#### 6.0.5 Update

Section 9.3：如果函数声明的返回值不是`void`，就必须有`return`语句返回函数返回值，反之~~语法错误~~**（修改为未定义行为）**。

Section 9.3：如果函数声明的返回值是`void`，`return`语句后必须单独出现表示从此位置跳出函数（`return;`），反之语法错误。**（补充解释：该语法错误意思为返回值为`void`的函数只能单独使用，即`return;`，任何`return <expr>;`均为语法错误）**

Section 9.3：删除：特别地，在Semantic Check阶段，为了简化流程，只要检查某个函数中是否存在`return`语句并且对应的返回值正确即视为语法正确。

Section 9.3：增加：在Semantic Check阶段，返回类型非`void`的返回值需要检查函数中每一个`return`语句的返回类型是否正确，作为语法检查。

#### 6.0.4 Update

Section 14: Update：简单来说，变量和函数是可以重名的，但是类是不可以和变量重名的。

Section 9.3: 补充：返回值可以是自定义类，可以是数组，可以是自定义类的数组。对于数组类型的返回值需要检查维数是否正确，也就是2维数组不可以返回给1维数组，保证维度上长度一定是匹配的。

#### 6.0.3 Update

Section 10.2：特殊的是：自增自减运算符在前缀加意义下表达式返回本身的值`+1`或`-1`的值。~~后缀加/减返回`void`~~**（更新：后缀加/减返回`int`）**